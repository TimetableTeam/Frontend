...
export default function SystemAdministration({ currentUser, initialTab = 'accounts', roleFilter = 'all' }) {
  const { t } = useLanguage()
  ...
  async function load() {
    ...
    setError('')
    try {
      const [accountsPayload, rolesPayload, departmentsPayload] = await Promise.all([
        tanseekApi.getAdminAccounts(),
        tanseekApi.getAdminRoles(),
        tanseekApi.getDepartments(),
      ])
  ...
  function startNewAccount() {
    ...
    const defaultRole = roleFilter !== 'all' ? roleFilter : (roles[0]?.id || '')
    setAccountForm({ ...emptyAccount, role: defaultRole })
  ...
  function editAccount(account) {
    ...
    setAccountForm({
      ...
      current_level: account.current_level || 1,
    })
    setAccountEditorOpen(true)
    setMessage('')
    setError('')
  ...
  async function saveAccount(event) {
    ...
    const departmentScopedRoles = ['admin', 'department_coordinator', 'lecturer', 'ta', 'student']
    if (departmentScopedRoles.includes(accountForm.role) && !accountForm.department_id) {
      setError(t('Select a department for this role.'))
      return
    }

    setSaving(true)
    try {
      const selectedDepartment = departments.find(item => String(item.id) === String(accountForm.department_id))
      const payload = {
        ...accountForm,
        department_id: accountForm.department_id ? Number(accountForm.department_id) : null,
        name: accountForm.name.trim(),
        email: accountForm.email.trim().toLowerCase(),
        department_name: selectedDepartment?.name || 'University-wide access',
      }
  ...
  const filteredAccounts = roleFilter === 'all'
    ? accounts
    : accounts.filter(account => normalizeRoleValue(account.role) === roleFilter)
  ...
  const accountViewTitle = {
    ...
    admin: 'Admins',
    department_coordinator: 'Department Coordinators',
    lecturer: 'Lecturers',
    ta: 'TAs',
    lab_manager: 'Lab Managers',
    registration_officer: 'Registration Officers',
    student: 'Students',
  }[roleFilter] || 'Accounts'

  const accountViewDescription = roleFilter === 'all'
    ? 'Create, edit, disable or remove user accounts and assign the correct role and scope.'
    : 'Filtered account list for the selected role.'

  return (
    <div>
      <PageHeader
        eyebrow={t('SUPER ADMIN')}
        title={t(tab === 'roles' ? 'Roles & permissions' : accountViewTitle)}
        description={t(tab === 'roles' ? 'Create custom roles and control which parts of Tanseek each role can access.' : accountViewDescription)}
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <Card className="p-4">
          ...
          <p className="mt-2 text-3xl font-bold text-tanseek-navy">{filteredAccounts.length}</p>
        </Card>
        <Card className="p-4">
            ...
            <div className="grid gap-5 xl:grid-cols-[1.3fr_0.7fr]">
              <Card className="overflow-hidden">
                <div className="border-b border-tanseek-line px-5 py-4">
                  <h2 className="font-bold text-tanseek-navy">{t(accountViewTitle)}</h2>
                  <p className="mt-1 text-xs text-tanseek-muted">{t('Manage university accounts by role. Student accounts are included and use their personal published timetable.')}</p>
                </div>

                {filteredAccounts.length === 0 ? (
                  <div className="p-8 text-center text-sm text-tanseek-muted">{t('No accounts found for this role.')}</div>
                ) : (
              ...
              <Card className="h-fit p-5">
                ...
                {!accountEditorOpen ? (
                  <div className="mt-6 rounded-brand-sm bg-tanseek-canvas p-4 text-sm leading-6 text-tanseek-muted">
                    {t('Select an account to edit it, or create a new account and assign one of the available roles.')}
                  ...
                  <form onSubmit={saveAccount} className="mt-5 space-y-4">
                    ...
                    <Field label={t('Department / scope')} hint={t(['admin', 'department_coordinator', 'lecturer', 'ta', 'student'].includes(accountForm.role) ? 'A department is required for this role.' : 'Leave university-wide for roles that are not department-scoped.')}>
                      <select value={accountForm.department_id || ''} onChange={e => { const department = departments.find(item => String(item.id) === String(e.target.value)); setAccountForm({ ...accountForm, department_id: e.target.value, department_name: department?.name || '' }) }} className="h-11 w-full rounded-brand-sm border border-tanseek-line bg-white px-3 text-sm text-tanseek-ink outline-none focus:border-tanseek-teal">
                        <option value="">{t('University-wide access')}</option>
                        {departments.filter(item => item.active !== false).map(department => <option key={department.id} value={department.id}>{department.code} — {department.name}</option>)}
                      </select>
                    </Field>
                    {accountForm.role === 'student' && (
                      <div className="grid gap-4 sm:grid-cols-2">
                        <Field label={t('University ID')}>
                          <input value={accountForm.university_id} onChange={e => setAccountForm({ ...accountForm, university_id: e.target.value })} placeholder="202400123" className="h-11 w-full rounded-brand-sm border border-tanseek-line bg-white px-3 text-sm text-tanseek-ink outline-none focus:border-tanseek-teal" dir="ltr" />
                        </Field>
                        <Field label={t('Current level')}>
                          <select value={accountForm.current_level} onChange={e => setAccountForm({ ...accountForm, current_level: Number(e.target.value) })} className="h-11 w-full rounded-brand-sm border border-tanseek-line bg-white px-3 text-sm text-tanseek-ink outline-none focus:border-tanseek-teal">
                            {[1,2,3,4].map(level => <option key={level} value={level}>{t('Level')} {level}</option>)}
                          </select>
                        </Field>
                      </div>
                    )}
