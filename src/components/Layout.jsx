...
const allNav = [
  ...
  { id: 'courses', label: 'Courses', icon: 'database' },
  { id: 'requirements', label: 'Requirements', icon: 'database' },
  { id: 'sections', label: 'Sections', icon: 'people' },
  { id: 'instructor_assignments', label: 'Instructor assignments', icon: 'user' },
  { id: 'section_assignments', label: 'Student section assignments', icon: 'people' },
  ...
  { id: 'registrations', label: 'Course registrations', icon: 'people' },
  { id: 'timetable', label: 'Timetable', icon: 'calendar' },
  { id: 'conflicts', label: 'Conflicts', icon: 'alert' },
]

const superAdminAccountNav = [
  { id: 'users_all', label: 'All accounts', icon: 'people' },
  { id: 'users_super_admin', label: 'Super Admins', icon: 'shield' },
  { id: 'users_admin', label: 'Admins', icon: 'user' },
  { id: 'users_scheduler', label: 'Schedulers', icon: 'calendar' },
  { id: 'users_registration_officer', label: 'Registration Officers', icon: 'people' },
  { id: 'users_coordinator', label: 'Department Coordinators', icon: 'database' },
  { id: 'users_lab_manager', label: 'Lab Managers', icon: 'equipment' },
  { id: 'users_lecturer', label: 'Lecturers', icon: 'user' },
  { id: 'users_ta', label: 'TAs', icon: 'user' },
  { id: 'users_student', label: 'Students', icon: 'people' },
]

function NavButton({ item, active, conflictCount, onClick, t }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      {item.id === 'conflicts' && conflictCount > 0 && (
        <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${active ? 'bg-tanseek-alertSoft text-tanseek-alert' : 'bg-tanseek-alert text-white'}`}>
          {conflictCount}
        </span>
      )}
    </button>
...
export default function Layout({ children, page, onPageChange, session, conflictCount = 0, onLogout, termName = '' }) {
  ...
  const superAdminUsersNav = useMemo(() => superAdminAccountNav.filter(item => canAccessPage(user, item.id)), [user.role, user.permissions])
  const superAdminOpsNav = useMemo(() => allNav.filter(item => ['courses', 'requirements', 'sections', 'instructor_assignments', 'section_assignments', 'labchecks', 'rooms', 'registrations', 'timetable', 'conflicts'].includes(item.id) && canAccessPage(user, item.id)), [user.role, user.permissions])
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-tanseek-canvas text-tanseek-ink">
      ...
      <aside className={`custom-scrollbar fixed inset-y-0 left-0 z-40 flex w-[264px] flex-col overflow-y-auto bg-tanseek-navy px-5 py-6 text-white transition-transform lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        {role === 'super_admin' ? (
          <>
            <div className="mt-10 text-[11px] font-bold uppercase tracking-[0.16em] text-white/40">{t('System')}</div>
            <nav className="mt-3 space-y-1">
              {canAccessPage(user, 'overview') && (
                <NavButton item={{ id: 'overview', label: 'Overview', icon: 'grid' }} active={page === 'overview'} conflictCount={conflictCount} onClick={() => go('overview')} t={t} />
              )}
            </nav>

            <div className="mt-7 text-[11px] font-bold uppercase tracking-[0.16em] text-white/40">{t('System setup')}</div>
            <nav className="mt-3 space-y-1">
              {canAccessPage(user, 'terms') && (
                <NavButton item={{ id: 'terms', label: 'Academic terms', icon: 'calendar' }} active={page === 'terms'} conflictCount={conflictCount} onClick={() => go('terms')} t={t} />
              )}
              {canAccessPage(user, 'departments') && (
                <NavButton item={{ id: 'departments', label: 'Departments', icon: 'room' }} active={page === 'departments'} conflictCount={conflictCount} onClick={() => go('departments')} t={t} />
              )}
            </nav>

            <div className="mt-7 text-[11px] font-bold uppercase tracking-[0.16em] text-white/40">{t('Operations')}</div>
            <nav className="mt-3 space-y-1">
              {superAdminOpsNav.map(item => (
                <NavButton key={item.id} item={item} active={page === item.id} conflictCount={conflictCount} onClick={() => go(item.id)} t={t} />
              ))}
            </nav>

            <div className="mt-7 text-[11px] font-bold uppercase tracking-[0.16em] text-white/40">{t('User management')}</div>
            <nav className="mt-3 space-y-1">
              {superAdminUsersNav.map(item => (
                <NavButton key={item.id} item={item} active={page === item.id} conflictCount={conflictCount} onClick={() => go(item.id)} t={t} />
              ))}
            </nav>

            <div className="mt-7 text-[11px] font-bold uppercase tracking-[0.16em] text-white/40">{t('Access control')}</div>
            <nav className="mt-3 space-y-1">
              {canAccessPage(user, 'roles') && (
                <NavButton item={{ id: 'roles', label: 'Roles & permissions', icon: 'shield' }} active={page === 'roles'} conflictCount={conflictCount} onClick={() => go('roles')} t={t} />
              )}
              {canAccessPage(user, 'audit') && (
                <NavButton item={{ id: 'audit', label: 'Audit log', icon: 'clock' }} active={page === 'audit'} conflictCount={conflictCount} onClick={() => go('audit')} t={t} />
              )}
            </nav>
          </>
        ) : (
              ...
              {userMenuOpen && (
                ...
                >
                  ...
                  <div className="border-t border-tanseek-line p-2">
                    <button
                      type="button"
                      onClick={() => { setUserMenuOpen(false); onPageChange('profile') }}
                      className="flex w-full items-center gap-3 rounded-brand-sm px-3 py-2.5 text-sm font-medium text-tanseek-navy transition hover:bg-tanseek-canvas"
                    >
                    ...
                    <button
                      ...
                      className="mt-1 flex w-full items-center gap-3 rounded-brand-sm px-3 py-2.5 text-sm font-medium text-tanseek-alert transition hover:bg-tanseek-alertSoft"
                    >
