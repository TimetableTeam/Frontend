...
export default function RoomsLabs({ readOnly = false }) {
  ...
  return (
    <>
      <PageHeader eyebrow={t('Facilities inventory')} title={t('Rooms & labs')} description={t(readOnly ? 'View room and lab capacity, equipment and availability while building the schedule. Facilities changes are managed by the Lab Manager.' : 'Search spaces by capacity, type, equipment and current availability. Closure and maintenance states remain visible in scheduling decisions.')} actions={readOnly ? null : <Button variant="secondary" icon="plus" onClick={openNew}>{t('Add space')}</Button>} />
      ...
      {loading ? (
        ...
        <Card className="overflow-hidden">
          ...
          {filtered.length === 0 ? (
            <div className="grid min-h-[260px] place-items-center p-8 text-center"><div><div className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-tanseek-navySoft text-tanseek-navy"><Icon name="room" size={23}/></div><h3 className="mt-4 text-base font-bold text-tanseek-navy">{t('No spaces found')}</h3><p className="mt-2 text-sm text-tanseek-muted">{t(readOnly ? 'No spaces match the current filters.' : 'Add a room or lab, or change the current filters.')}</p>{!readOnly && <Button className="mt-5" icon="plus" onClick={openNew}>{t('Add space')}</Button>}</div></div>
          ) : (
      ...
      {!readOnly && <SpaceModal open={modalOpen} room={selected} onClose={()=>{setModalOpen(false);setSelected(null)}} onSaved={handleSaved} onDeleted={handleDeleted}/>}
    </>
  )
}
