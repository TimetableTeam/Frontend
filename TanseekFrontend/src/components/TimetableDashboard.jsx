  ...
  return (
    <Card className="grid min-h-[360px] place-items-center p-10 text-center">
      <div className="max-w-md">
        ...
        <p className="mt-2 text-sm leading-6 text-tanseek-muted">{t('The scheduler has not published an official timetable for this term yet. Draft allocations are not visible here.')}</p>
      </div>
    </Card>
  )
}

function PublishModal({ mode, conflictCount, allocationCount, publishedVersion, publishing, error, onClose, onReviewConflicts, onConfirm }) {
      ...
      <div className="w-full max-w-[540px] overflow-hidden rounded-brand border border-tanseek-line bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-4 border-b border-tanseek-line p-5 md:p-6">
          <div className="flex gap-3">
            <div className={`grid h-11 w-11 shrink-0 place-items-center rounded-brand-sm ${blocked ? 'bg-tanseek-alertSoft text-tanseek-alert' : success ? 'bg-tanseek-tealSoft text-tanseek-teal' : 'bg-tanseek-navySoft text-tanseek-navy'}`}>
              <Icon name={blocked ? 'alert' : success ? 'check' : 'calendar'} size={22} />
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-tanseek-muted">Fall 2026</p>
              <h2 className="mt-1 text-xl font-bold text-tanseek-navy">{blocked ? t('Cannot publish this version') : success ? t('Timetable published') : t('Publish timetable?')}</h2>
            </div>
          </div>
        ...
        <div className="p-5 md:p-6">
          {blocked ? (
            ...
            <>
              ...
              <p className="mt-4 text-xs leading-5 text-tanseek-muted">{language === 'ar' ? 'سجّل الدخول بحساب دكتور/معيد للتأكد من ظهور النسخة المنشورة فقط. ويمكن للطلاب رؤيتها من الصفحة الرئيسية بدون تسجيل دخول.' : 'Sign in with the Doctor/TA demo account to verify the published staff view. Students can view the published timetable from the public home page without signing in.'}</p>
            </>
          ) : (
            <>
              <p className="text-sm leading-6 text-tanseek-ink">{language === 'ar' ? 'سيتم إنشاء نسخة رسمية منشورة جديدة من المسودة الحالية. سيعرض الدكتور/المعيد هذه النسخة داخل حسابه، وسيعرضها الطلاب من الصفحة الرئيسية.' : `This creates a new official published version from the current draft. Doctor/TA views and the public student timetable will read this snapshot instead of draft data.`}</p>
...
export default function TimetableDashboard({
  ...
  const publishedOnly = role === 'lecturer'
  ...
  const baseAllocations = useMemo(() => {
    ...
    if (role === 'lecturer') return published.filter(item => item.staff === user?.name)
    return published
  ...
  const description = publishedOnly
    ? t('Latest official published schedule. Draft changes are hidden until the scheduler publishes a new version.')
    ...
    <>
      <PageHeader
        ...
        actions={!publishedOnly && <>
          ...
          <Button variant="secondary" icon="filter" onClick={() => setFiltersOpen(prev => !prev)}>
            {t('Filters')}{activeFilterCount > 0 ? ` (${activeFilterCount})` : ''}
          </Button>
          {canPublish && <Button onClick={openPublish}>{t('Publish version')}</Button>}
        </>}
      />
