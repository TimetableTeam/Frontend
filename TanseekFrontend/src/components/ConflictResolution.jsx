...
export default function ConflictResolution({ conflicts, resolvedIds, onResolve }) {
  ...
  if (!conflict) {
    return <><PageHeader eyebrow={t('Validation engine')} title={t('Conflict resolution')} description={t('Hard conflicts are checked before publication.')}/><Card className="grid min-h-[360px] place-items-center p-10 text-center"><div><div className="mx-auto grid h-14 w-14 place-items-center rounded-full bg-tanseek-tealSoft text-tanseek-teal"><Icon name="check" size={28}/></div><h2 className="mt-5 text-xl font-bold text-tanseek-navy">{t('No hard conflicts remain')}</h2><p className="mt-2 max-w-md text-sm leading-6 text-tanseek-muted">{t('The current draft is clear of blocking conflicts in this demo dataset and can now be published from the Timetable page.')}</p></div></Card></>
  }
      ...
      <div className="grid gap-6 xl:grid-cols-[330px_1fr]">
        <Card className="overflow-hidden self-start">
          ...
          <div className="divide-y divide-tanseek-line">
            {unresolved.map(c => <button key={c.id} onClick={()=>{setSelectedId(c.id);setSelectedAlternative(null)}} className={`w-full p-4 text-left transition ${conflict.id===c.id ? 'bg-tanseek-alertSoft' : 'bg-white hover:bg-tanseek-canvas'}`}><div className="flex items-start gap-3"><span className="mt-0.5 text-tanseek-alert"><Icon name="alert" size={18}/></span><span className="min-w-0"><span className="block text-sm font-bold text-tanseek-ink">{t(c.title)}</span><span className="mt-1 block text-xs leading-5 text-tanseek-muted">{t(c.scope)}</span></span></div></button>)}
          </div>
        </Card>
        ...
        <div className="space-y-6">
          ...
          <Card className="overflow-hidden">
            <div className="border-b border-tanseek-line p-5 md:px-6"><h2 className="text-lg font-bold text-tanseek-navy">{t('Suggested spaces & slots')}</h2><p className="mt-1 text-sm text-tanseek-muted">{t('Ranked by hard feasibility first, then capacity fit, equipment match, preferred windows and utilization.')}</p></div>
            <div className="divide-y divide-tanseek-line">
              {conflict.alternatives.map((a, index) => {
                ...
                return <button key={a.id} onClick={()=>setSelectedAlternative(a.id)} className={`flex w-full flex-col gap-4 p-5 text-left transition md:flex-row md:items-center ${selected ? 'bg-tanseek-tealSoft/70' : 'hover:bg-tanseek-canvas/80'}`}>
                  <div className="flex flex-1 items-start gap-4"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-brand-sm text-sm font-bold ${index===0?'bg-tanseek-teal text-tanseek-navy':'bg-tanseek-navySoft text-tanseek-navy'}`}>{index+1}</div><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-base font-bold text-tanseek-navy">{a.room}</h3>{index===0 && <StatusBadge status="available">{t('Best fit')}</StatusBadge>}</div><p className="mt-1 text-sm text-tanseek-muted">{t(a.day)} · {a.time} · {a.capacity} {t('seats')}</p><p className="mt-2 text-xs leading-5 text-tanseek-muted">{t(a.equipment)} · {t(a.tradeoff)}</p></div></div>
                  <div className="flex items-center gap-4 md:justify-end"><div className="text-right"><p className="text-[11px] uppercase tracking-[0.08em] text-tanseek-muted">{t('Fit score')}</p><p className="text-xl font-bold text-tanseek-navy">{a.score}</p></div><div className={`grid h-9 w-9 place-items-center rounded-full border ${selected?'border-tanseek-teal bg-tanseek-teal text-tanseek-navy':'border-tanseek-line bg-white text-transparent'}`}><Icon name="check" size={17}/></div></div>
                </button>
              })}
            </div>
            <div className="flex flex-col gap-3 border-t border-tanseek-line bg-tanseek-canvas/60 p-5 sm:flex-row sm:items-center sm:justify-between md:px-6"><p className="text-xs text-tanseek-muted">{t('Scheduler confirmation is required; recommendations never auto-publish.')}</p><Button disabled={!selectedAlternative} onClick={()=>{onResolve(conflict.id, selectedAlternative);setSelectedAlternative(null)}} className={!selectedAlternative?'cursor-not-allowed opacity-45':''} icon="check">{t('Apply selected alternative')}</Button></div>
          </Card>
        </div>
      </div>
    </>
