...
export default function LabManagerChecks() {
  ...
  const section = selected && catalog?.sections.find(s => Number(s.id) === Number(selected.section_id))
  ...
  return (
    <>
      ...
      <div className="grid gap-6 2xl:grid-cols-[390px_1fr]">
        <Card className="self-start overflow-hidden"><div className="border-b border-tanseek-line p-5"><h2 className="text-lg font-bold text-tanseek-navy">{t('Lab sessions')}</h2><p className="mt-1 text-xs text-tanseek-muted">{t('Requirements needing facilities review')}</p></div><div className="divide-y divide-tanseek-line">{loading && <div className="p-5 text-sm text-tanseek-muted">{t('Loading lab checks…')}</div>}{!loading && data.requirements.map(req => { const c = catalog?.courses.find(x => Number(x.id) === Number(req.course_id)); const sec = catalog?.sections.find(x => Number(x.id) === Number(req.section_id)); return <button key={req.id} onClick={() => { setSelectedId(req.id); setMessage(''); setNotes(req.decision?.notes || '') }} className={`w-full p-5 text-left ${Number(selectedId)===Number(req.id)?'bg-tanseek-navySoft':'hover:bg-tanseek-canvas'}`}><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold text-tanseek-navy">{c?.code} · {t(req.session_type)}</p><p className="mt-1 text-xs text-tanseek-muted">{sec?.name}</p></div><StatusBadge status={req.decision ? 'confirmed' : req.state === 'READY' ? 'ready' : 'pending'}>{req.decision ? t('Checked') : req.state === 'READY' ? t('Ready') : t('Incomplete')}</StatusBadge></div><p className="mt-3 text-xs text-tanseek-muted">{t(req.required_room_type)} · {(req.required_equipment || []).length} {t('equipment needs')}</p></button>})}</div></Card>

        {selected ? <div className="space-y-6">
          <Card className="p-5 md:p-6"><div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.12em] text-tanseek-teal">{t('Requirement')} #{selected.id}</p><h2 className="mt-1 text-xl font-bold text-tanseek-navy">{course?.code} — {course?.name}</h2><p className="mt-1 text-sm text-tanseek-muted">{section?.name}</p></div>{selected.decision && <StatusBadge status="confirmed">{t('Confirmed')}: {data.rooms.find(r=>r.id===selected.decision.room_id)?.name || selected.decision.room_id}</StatusBadge>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div className="rounded-brand-sm bg-tanseek-canvas p-3"><p className="text-[11px] text-tanseek-muted">{t('Duration')}</p><p className="mt-1 text-sm font-bold text-tanseek-navy">{selected.duration_minutes} min</p></div><div className="rounded-brand-sm bg-tanseek-canvas p-3"><p className="text-[11px] text-tanseek-muted">{t('Room type')}</p><p className="mt-1 text-sm font-bold text-tanseek-navy">{t(selected.required_room_type)}</p></div><div className="rounded-brand-sm bg-tanseek-canvas p-3 sm:col-span-2"><p className="text-[11px] text-tanseek-muted">{t('Equipment')}</p><p className="mt-1 text-sm font-bold text-tanseek-navy">{(selected.required_equipment || []).map(equipmentLabel).join(' · ') || t('None')}</p></div></div>{selected.decision && <div className="mt-4 flex items-start gap-3 rounded-brand-sm border border-tanseek-teal/25 bg-tanseek-tealSoft p-4"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-tanseek-teal text-white"><Icon name="check" size={16}/></span><div><p className="text-sm font-bold text-tanseek-navy">{t('Lab confirmation saved')}</p><p className="mt-1 text-xs leading-5 text-tanseek-muted">{t('Confirmed lab:')} {data.rooms.find(r=>r.id===selected.decision.room_id)?.name || selected.decision.room_id}{selected.decision.notes ? ` · ${selected.decision.notes}` : ''}</p></div></div>}{selected.notes && <div className="mt-4 rounded-brand-sm border border-tanseek-line p-4 text-xs leading-5 text-tanseek-muted"><span className="font-bold text-tanseek-ink">{t('Coordinator note:')} </span>{selected.notes}</div>}</Card>
